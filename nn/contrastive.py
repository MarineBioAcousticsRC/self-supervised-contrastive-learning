...
#
# Self-supervised contrastive finetuning: dataset, CSV parsing, triplet loss.
#
# CSV labels define vocalization time intervals. Species names are used only
# to drop Empty/Unknown/silence rows, not to train a classifier.
# Intervals are mapped from wav samples to encoder frames so triplets are
...
logger = logging.getLogger("ssl_contrastive")
...
def sample_class_aware_positive(
    ...
    Unused in training. Kept for diagnostics. The supervised contrastive control uses
    same-span positives only; labels affect negative sampling, not positives.
