    ...
    with torch.inference_mode():
        ...
        for samples in tqdm(dataloader, total=len(dataset.fnames),
            ...
            if args.write_embeddings:
                ...
                emb_h5_path = os.path.join(
                    args.out_path,
                    "{}_embeddings_{}_{}_{}.h5".format(*file_args),
                )
            ...
            if not args.overwrite_previous_preds:
                # Embedding-only runs: skip when .h5 exists (not header-only CSV from a prior pass).
                if args.write_embeddings and emb_h5_path and os.path.isfile(emb_h5_path):
                    continue
                if write_secondary_pred_file and file_exists and secondary_file_exists:
                    continue
                elif not args.write_embeddings and file_exists:
                    continue

            if args.write_embeddings:
                out_path = emb_h5_path
