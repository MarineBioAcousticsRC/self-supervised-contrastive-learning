...
#
# Self-supervised contrastive finetune for pretrained animal2vec checkpoints.
#
# Training recipe:
#   - Student: pretrained animal2vec, updated by AdamW
#   - Data: Fairseq manifest (wav paths) + CSV vocalization intervals
#   - Forward: extract_features -> average top-K layer embeddings (B, T, D)
#   - Loss: triplet margin on vocal vs non-vocal / other-recording frames
...
logger = logging.getLogger("ssl_contrastive_finetune")
...
def save_fairseq_checkpoint(path: str, student, pretrain_ckpt: str, num_updates: int):
    ...
    extra["ssl_contrastive_finetune"] = True
...
def train(args):
    ...
    while step < args.max_updates:
        ...
        loss, stats = compute_contrastive_loss(
            ...
            margin=args.margin,
            rng=rng,
            class_aware=args.supervised_contrastive,
